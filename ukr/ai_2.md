...
Вартість самоуваги зростає квадратично зі збільшенням довжини контексту: удвічі довше вікно - приблизно вчетверо більше обчислень і пам’яті. Тому розширення контексту потребує інженерних прийомів: від відносних позиційних кодувань і буферизації до спарс- або локальної уваги. Частково це вдається покращувати приблизними алгоритмами, наприклад sliding window/local attention, sparse attention у стилі Longformer або BigBird, а також різними варіантами linear attention на кшталт Performer. Окремо варто згадати FlashAttention: це не приблизний алгоритм, а дуже вдалий IO-aware спосіб рахувати точний attention із меншими накладними витратами на пам'ять і доступ до неї. Але і він не прибирає фундаментальну проблему повністю. Усе це лише часткові компроміси, а не повне вирішення проблеми. У прикладних системах це доповнюють зовнішнім пошуком знань (RAG - Retrieval-Augmented Generation, пошук інформації в базах даних + генерація) і кешуванням, аби не тягнути в модель усе одразу.

_Оновлення станом на весну 2026:_ ця проблема все ще стоїть дуже гостро: моделі як і раніше залишаються класичними трансформерами без нативних STM та LTM і все ще намагаються компенсувати це довжиною контексту, яку суттєво збільшувати досі не вдається.

## Недоліки та перспективи подальшого розвитку

Попри проривні можливості роботи з мовою, LLM мають системні обмеження. Вони схильні до "галюцинацій" - упевненої генерації неправди у зонах невизначеності; чутливі до формулювань підказок; не мають вбудованої перевірки фактів. Найбільш відчутним є дефіцит справжньої довготривалої пам'яті: все, що виходить за межі вікна контексту, втрачається, тож модель не "пам'ятає" користувача чи довгий процес у строгому сенсі.

_Оновлення станом на весну 2026:_ на рівні самих моделей це залишається майже без змін: проблеми відсутності STM та LTM не вирішені, а збільшення контексту саме по собі не стало достатньою компенсацією. Також мають значення вартість і енергоспоживання, упередження, питання безпеки й конфіденційності, обмежене планування та міркування без допоміжних інструментів.

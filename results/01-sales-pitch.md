...
**4. The regulatory wave.** The EU AI Act becomes fully applicable **August 2, 2026 — roughly 80 days away**. Standard software engineering workflows with PR reviews are exempt from watermarking obligations (Article 50 exemption for human-reviewed content). But organizations using AI in regulated products (medical, financial, critical infrastructure) face "high-risk" classification requiring documented human oversight. On the IP front: AI-generated code without significant human creative modification is not copyrightable (US Copyright Office, 2025). Engineers must actively reshape AI output — not just review and approve it — to claim IP protection. SDD's emphasis on human architectural decisions provides the strongest framework for this.
...
**IP reminder:** Merely prompting an AI and reviewing its output does not confer copyright — engineers must make creative modifications to the generated code. SDD provides the strongest foundation for IP protection.
...
**The specification repository becomes core IP.** Not the code. Code can be regenerated from spec; institutional knowledge encoded in specifications cannot be shortcut. The spec repository becomes the primary source of truth from which artifacts — docs, designs, code, plans — can be regenerated. Since AI-generated code without human creative modification isn't copyrightable, the human-authored specification — plus active engineering of the output — is your strongest IP asset.
...
**Plan for the productivity dip.** Research consistently shows 20-30% development cost reductions for organizations that deeply restructure workflows — but these gains require genuine organizational redesign, not just tool provisioning. Expect a 3-6 month calibration period. Organizations that expect instant results fail; those that invest in the transition compound.
...
- **Ground every claim in defensible numbers:** 2-5x implementation speed for well-specified tasks, 50% spec time, $10-200/month tooling cost range (pro to enterprise), 2-4 week break-even. Detailed sources and evidence are in the [Fact-Check Report](../docs/fact-check-2026.md)
